...
const summary = async (text: string): Promise<string> => {
  await sleep(4_000);
...
const timeSeries = async (searchText: string): Promise<{
  from: Date;
  statistics: {
    tstamp: string;
    significant: number;
  ...
  const from: Date = lookback();
  const points: Point[] = d3fc
    .randomFinancial()
    .startDate(from)
    .startPrice(1)
    .interval(d3.timeHour)
    .stream()
    .until((d: { date: Date; close: number; open: number }) => d.date > new Date())
    .map((d: { date: Date; close: number; open: number }) => {
      const obj: Point = {
        tstamp: d.date.toISOString(),
      ...
      if (Math.random() >= 0.995) {
        obj.significant = {
          id: `dummy:${+d.date}`,
          published: new Date(+d.date - Math.random() * 1000 * 60 * 60 * 2).toISOString(),
